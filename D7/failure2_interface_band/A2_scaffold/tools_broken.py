...
the whole change is the signature of get_clinic_slots:

    def get_clinic_slots(specialty, band, **window):        # tools.py
    def get_clinic_slots(specialty, band="routine", **window):  # here

tools.py's own docstring for this function calls the deleted part out
by name: "band IS A REQUIRED ARGUMENT, and this is the clearest
example in the scaffold of designing an interface so the wrong call
cannot be made." A call that forgets `band` no longer fails to
...
def get_clinic_slots(specialty, band="routine", **window):   # DELETED: band required
